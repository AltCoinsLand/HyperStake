...
bool stakeTargetHit(uint256 hashProofOfStake, unsigned int nTxTime, unsigned int nPrevTxTime, int64 nValueIn, CBigNum bnTargetPerCoinDay, uint256 hashBlockFrom)
{	
	//get the stake weight
	int64 nTimeWeight = min((int64)(nTxTime - nPrevTxTime), (int64)nStakeMaxAge) - nStakeMinAge;
	CBigNum bnCoinDayWeight = 0;
	if(nTxTime < FORK_TIME2)		
		bnCoinDayWeight = CBigNum(nValueIn) * nTimeWeight / COIN / (24 * 60 * 60);
	else
	{
		if (!mapBlockIndex.count(hashBlockFrom))
			return error("stakeTargetHit() : block not indexed");
		const CBlockIndex* pindexFrom = mapBlockIndex[hashBlockFrom];
		bnCoinDayWeight = CBigNum(nValueIn) * nTimeWeight / COIN / (24 * 60 * 60);
		if(bnCoinDayWeight > CBigNum(pindexFrom->nMoneySupply / COIN / 500))
		{
			bnCoinDayWeight = CBigNum(pindexFrom->nMoneySupply / COIN / 500);
			printf("****COIN WEIGHT CAPPED****");
		}
	}
	...
	const COutPoint& prevout, unsigned int& nTimeTx, unsigned int nHashDrift, bool fCheck, uint256& hashProofOfStake, bool fPrintProofOfStake)
{
	...
	if(fCheck)
	{
		...
		return stakeTargetHit(hashProofOfStake, nTimeTx, nTxPrevTime, nValueIn, bnTargetPerCoinDay, blockFrom.GetHash());
	}
	
	nHashDrift = min(nHashDrift, (unsigned int)(60)); // max clock drift is 60 seconds
    bool fSuccess = false;
	...
	for(i = 0; i < (nHashDrift*2); i++) //iterate the hashing
	{
		...
		if(!stakeTargetHit(hashProofOfStake, nTimeTx, nTxPrevTime, nValueIn, bnTargetPerCoinDay, blockFrom.GetHash()))
			continue;
